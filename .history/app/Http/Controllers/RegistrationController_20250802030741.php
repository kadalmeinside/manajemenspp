    ...
    public function store(Request $request, XenditService $xenditService)
    {
        ...
        try {
            $invoiceUrl = DB::transaction(function () use ($validated, $kelas, $xenditService, $request) {
                // 1. Hitung biaya pendaftaran final di backend untuk keamanan
                ...
                $invoice = Invoice::create([
                    ...
                    'due_date' => now()->addMinutes(30), // Beri waktu 30 menit untuk bayar
                    'status' => 'PENDING',
                    'external_id_xendit' => 'REG-'.$siswa->id_siswa.'-'.strtoupper(Str::random(6)),
                ]);

                // 5. Buat Link Pembayaran Xendit
                $payerInfo = ['email' => $user->email, 'name' => $user->name, 'phone' => $siswa->nomor_telepon_wali];
                $xenditInvoiceData = $xenditService->createInvoice($biayaFinal, $adminFee, $deskripsi, $payerInfo, $invoice->external_id_xendit, route('registration.success'), route('payment.failure'), now()->addMinutes(30), ['email', 'whatsapp']);
