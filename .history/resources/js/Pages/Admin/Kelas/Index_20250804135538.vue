...
// ### PERBAIKAN: Fungsi baru untuk menangani input mata uang secara real-time ###
const handleCurrencyInput = (event, formKey) => {
    ...
    // 3. Perbarui nilai di dalam form dengan angka murni
    if (isNaN(numericValue)) {
        form[formKey] = null;
        event.target.value = ''; // Kosongkan input jika tidak ada angka
    } else {
        form[formKey] = numericValue;
        // 4. Format angka dengan titik pemisah dan perbarui tampilan input
        event.target.value = new Intl.NumberFormat('id-ID').format(numericValue);
    }
};

// Computed property untuk menampilkan nilai yang sudah diformat saat modal edit dibuka
const biayaSppFormattedOnOpen = computed(() => {
...
const biayaPendaftaranFormattedOnOpen = computed(() => {
            ...
            <div class="p-6">
                ...
                <form @submit.prevent="submitKelasForm" class="space-y-6">
                    ...
                    <div>
                        ...
                        <!-- ### PERBAIKAN: Menggunakan :value dan @input ### -->
                        <TextInput id="kelas_biaya_pendaftaran_modal" type="text" inputmode="numeric" class="mt-1 block w-full" 
                                   :value="biayaPendaftaranFormattedOnOpen" 
                                   @input="handleCurrencyInput($event, 'biaya_pendaftaran')" 
                    ...
                    <div>
                        ...
                        <TextInput id="kelas_biaya_spp_modal" type="text" inputmode="numeric" class="mt-1 block w-full" 
                                   :value="biayaSppFormattedOnOpen" 
                                   @input="handleCurrencyInput($event, 'biaya_spp_default')" 
