...
// ### PERBAIKAN: Fungsi untuk memblokir input non-numerik ###
const onCurrencyKeyDown = (event) => {
    // Izinkan tombol kontrol: backspace, delete, tab, escape, enter
    if ([46, 8, 9, 27, 13].indexOf(event.keyCode) !== -1 ||
        // Izinkan: Ctrl+A, Command+A
        (event.keyCode === 65 && (event.ctrlKey === true || event.metaKey === true)) ||
        // Izinkan: Ctrl+C, Command+C
        (event.keyCode === 67 && (event.ctrlKey === true || event.metaKey === true)) ||
        // Izinkan: Ctrl+V, Command+V
        (event.keyCode === 86 && (event.ctrlKey === true || event.metaKey === true)) ||
        // Izinkan: Ctrl+X, Command+X
        (event.keyCode === 88 && (event.ctrlKey === true || event.metaKey === true)) ||
        // Izinkan: home, end, left, right, down, up
        (event.keyCode >= 35 && event.keyCode <= 40)) {
        // Biarkan tombol berfungsi normal
        return;
    }
    // Pastikan itu adalah angka dan hentikan jika bukan
    if ((event.shiftKey || (event.keyCode < 48 || event.keyCode > 57)) && (event.keyCode < 96 || event.keyCode > 105)) {
        event.preventDefault();
    }
};

// Computed property untuk formatting input Rupiah
