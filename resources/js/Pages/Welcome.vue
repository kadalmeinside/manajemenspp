...
const hideLoader = inject('hideLoader', () => console.error('Inject hideLoader gagal!'));
...
const programs = ref([
    ...
    { title: 'Soccer School', description: 'Pembinaan fundamental yang menyenangkan untuk semua level usia.', icon: UsersIcon, href: '#', color: 'text-amber-600', bgColor: 'bg-amber-50' },
    { title: 'Young Tiger League', description: 'Kompetisi internal untuk mengasah mental bertanding para siswa.', icon: TrophyIcon, href: '#', color: 'text-indigo-600', bgColor: 'bg-indigo-50' }
]);
            ...
            <main class="bg-white text-zinc-800">
                <section class="relative h-screen w-full flex items-center justify-center text-center overflow-hidden">
                    <transition-group name="fade">
                        <div class="absolute inset-0 h-full w-full" v-for="(slide, index) in heroSlides" :key="index" v-show="index === currentHeroSlide">
                    ...
                    <div class="relative z-20 px-4 flex flex-col items-center">
                        <transition name="slide-up" mode="out-in">
                            <div :key="currentHeroSlide" class="flex flex-col items-center">
                ...
                <section id="program" class="py-16 md:py-24">
                    <div class="container-app">
                        ...
                        <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                            <div v-for="(program, index) in programs" :key="program.title"
                                 v-animate-on-scroll
                                 class="program-summary-card animate-card"
                    ...
                    <div class="container-app">
                        ...
                        <div class="mt-12">
                            <div ref="schoolSliderContainer" class="card-container-slider">
                                <div v-for="(cabang, index) in soccerSchoolBranches" :key="cabang.id"
                                     v-animate-on-scroll
                ...
                <section id="academy-details" class="py-16 md:py-24">
                    <div class="container-app">
                        ...
                        <div class="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                            <div v-animate-on-scroll class="feature-card animate-card" :style="{ transitionDelay: '0ms' }">
                            ...
                            <div v-animate-on-scroll class="feature-card animate-card" :style="{ transitionDelay: '100ms' }">
                            ...
                            <div v-animate-on-scroll class="feature-card animate-card" :style="{ transitionDelay: '200ms' }">
                ...
                <section id="berita" class="py-16 md:py-24 bg-gray-50">
                    <div class="container-app">
                        <div class="flex items-end justify-between border-b-2 border-gray-700 pb-2">
                            ...
                            <a href="#" class="text-sm font-semibold hover:text-red-500 transition">Lihat Semua →</a>
                        </div>
                        <div class="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            <template v-if="newsLoading">
                                <div v-for="i in 3" :key="i" class="news-card-skeleton">
                                    <div class="h-56 bg-gray-300 rounded-lg"></div>
                            ...
                            <template v-else>
                                <a v-for="(item, index) in news" :key="item.link"
                                   v-animate-on-scroll
                                   :href="item.link" target="_blank" rel="noopener"
                                   class="news-card group animate-card"
                                   :style="{ transitionDelay: `${index * 100}ms` }">
                 ...
                 <section id="gallery" class="py-16 md:py-24 bg-zinc-950 text-white">
                    <div class="container-app">
                        ...
                        <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                             <a v-for="(video, index) in videoGalleries" :key="video.link"
                               v-animate-on-scroll
                               :href="video.link" target="_blank" rel="noopener"
                               class="video-card group animate-card"
                               :style="{ transitionDelay: `${index * 100}ms` }">
...
.section-title { @apply font-header text-4xl font-bold text-zinc-900 md:text-5xl uppercase; }
.section-title.text-white { color: white; }
.section-subtitle { @apply mt-2 max-w-2xl mx-auto text-base text-zinc-600 md:text-lg; }
.program-summary-card { @apply bg-white p-6 rounded-xl border border-gray-100 transition-all duration-300 hover:shadow-xl hover:border-gray-200 hover:-translate-y-1; }
.program-summary-link { @apply inline-flex items-center gap-2 mt-4 text-sm font-semibold text-red-600 hover:underline; }
.slider-nav-button { @apply p-2 bg-white rounded-full shadow hover:bg-gray-100 text-zinc-600 hover:text-zinc-900 transition-all duration-200; }
.card-container-slider { @apply flex space-x-6 overflow-x-auto pb-4 scroll-smooth snap-x snap-mandatory; -ms-overflow-style: none; scrollbar-width: none; }
.card-container-slider::-webkit-scrollbar { display: none; }
.card { @apply relative w-80 h-[450px] rounded-2xl overflow-hidden shadow-xl transition-transform duration-300 flex-shrink-0 snap-center; }
.card:hover { @apply transform -translate-y-2; }
.card-background-image { @apply absolute inset-0 w-full h-full object-cover z-0 transition-all duration-500 ease-in-out; }
.card:hover .card-background-image { @apply filter blur-sm; }
.card-content { @apply relative z-10 flex flex-col justify-end h-full p-6 bg-gradient-to-t from-black/90 via-black/50 to-transparent transition-all duration-500; }
.card:hover .card-content { @apply from-black/90 via-black/30; }
.card-title { @apply font-header font-bold text-2xl leading-tight text-white; text-shadow: 1px 1px 5px rgb(0 0 0 / 0.5); }
.card-subtitle { @apply text-sm text-white/80 mt-1; text-shadow: 1px 1px 5px rgb(0 0 0 / 0.5); }
.card-button { @apply block w-full text-center py-3 px-6 rounded-lg font-semibold bg-white/10 border border-white/20 backdrop-blur-sm text-white hover:bg-white/20 transition-colors duration-300; }
.feature-card { @apply bg-white p-8 rounded-xl border border-gray-100 transition-all duration-300 hover:border-red-500/50 hover:shadow-xl hover:-translate-y-1; }
.feature-card-icon { @apply w-16 h-16 rounded-full bg-red-600/10 text-red-500 flex items-center justify-center mx-auto; }
.feature-card-title { @apply mt-5 font-bold text-lg text-zinc-900; }
.feature-card-desc { @apply mt-2 text-sm text-zinc-600; }
.news-card { @apply block bg-white rounded-lg overflow-hidden border border-gray-100 hover:border-gray-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300; }
.news-card-img { @apply h-56 w-full object-cover group-hover:scale-105 transition-transform duration-500; }
.news-card-title { @apply font-bold text-lg text-zinc-900 group-hover:text-red-600 transition-colors duration-300 leading-tight; }
.news-card-skeleton { @apply bg-white rounded-lg overflow-hidden border border-gray-100 animate-pulse; }
.video-card { @apply block relative overflow-hidden rounded-lg shadow-lg; }
.video-card-img { @apply w-full h-full object-cover transition-transform duration-500 group-hover:scale-105; }
.video-card-play-button { @apply w-16 h-16 bg-red-600/80 rounded-full flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:bg-red-600; }
.loader { width: 48px; height: 48px; border: 3px solid #FFF; border-bottom-color: #FF2D20; border-radius: 50%; display: inline-block; box-sizing: border-box; animation: rotation 1s linear infinite; }
